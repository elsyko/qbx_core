...
function CheckPlayerData(source, playerData)
    ...
    if source then
        ...
        playerData.name = GetPlayerName(source) or 'Unknown'
        Offline = false
    end
    ...
    playerData.charinfo.fullname = string.format("%s %s", playerData.charinfo.firstname, playerData.charinfo.lastname)
    ...
    playerData.metadata.creditscore = playerData.metadata.creditscore or 300
...
function CreatePlayer(playerData, Offline)
        ...
        if not self.Offline then
            ...
            if amount > 9999 then
                if moneytype == "cash" then
                    TriggerEvent("gotcha:cashLog", self.PlayerData.source, "add", amount, self.PlayerData.money[moneytype])
                elseif moneytype == "bank" then
                    TriggerEvent("gotcha:bankLog", self.PlayerData.source, "Deposit", amount, self.PlayerData.money[moneytype], reason or "N/A")
                end
            end
            ...
            if moneytype == "bank" then
                exports.fd_banking:handleDepositToPersonalAccount(self.PlayerData.source, amount, self.PlayerData.money[moneytype], reason)
            end
            TriggerClientEvent("inv-phone:sendNewAlert", self.PlayerData.source, {
                app = "Banking",
                icon = "fas fa-university",
                title = moneytype == "bank" and "Bank Account Deposit" or "Cash Received",
                message = string.format("$%s | %s", amount, reason or "N/A"),
                class = "success",
            })
        end
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if moneytype == "bank" then
            local isFrozen = exports.fd_banking:isPersonalAccountFrozenFromSource(self.PlayerData.source)
            if isFrozen then
                print("Bank account for CID " .. self.PlayerData.citizenid .. " is frozen and money was attempted to be deducted")
                return false
            end
        end
        ...
        if not self.Offline then
            ...
            if amount > 9999 then
                if moneytype == "cash" then
                    TriggerEvent("gotcha:cashLog", self.PlayerData.source, "remove", amount, self.PlayerData.money[moneytype])
                elseif moneytype == "bank" then
                    TriggerEvent("gotcha:bankLog", self.PlayerData.source, "Withdrawl", amount, self.PlayerData.money[moneytype], reason or "N/A")
                end
            end
            local tags = amount > 100000 and config.logging.role or nil
            logger.log({
            ...
            if moneytype == "bank" then
                exports.fd_banking:handleWithdrawFromPersonalAccount(self.PlayerData.source, amount, self.PlayerData.money[moneytype], reason)
            end
            TriggerClientEvent("inv-phone:sendNewAlert", self.PlayerData.source, {
                app = "Banking",
                icon = "fas fa-university",
                title = moneytype == "bank" and "Bank Account Withdrawl" or "Cash Taken",
                message = string.format("$%s | %s", amount, reason or "N/A"),
                class = "error",
            })
        end
...
function Save(source)
    ...
    if Player and not Player.inApartment and not Player.inProperty then
